...
@Component({
  selector: 'app-sorteo-final-liguilla',
  standalone: true,
  imports: [HeaderComponent, FormsModule, FontAwesomeModule, CommonModule],
  templateUrl: './sorteo-final-liguilla.component.html',
  styleUrl: './sorteo-final-liguilla.component.css'
})
export class SorteoFinalLiguillaComponent {
  equipos: string[] = [];  
  divisor: number = 0  ;
  ...
  completarPartido(grupoIndex: number, partidoIndex: number) {
    if (this.golesTemporales[`${grupoIndex}-${partidoIndex}`] === undefined){
      ...
      if (equipo1 && equipo2) {
        ...
        if (this.i < this.jornadas[0].length - 1){
          this.i++
        } else {
          this.equipoGanador = this.getNombrePrimerEquipo()
          this.toggleModal()
          this.i = 0
        }
    }
