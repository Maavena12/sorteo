...
@Component({
  selector: 'app-eliminatorias-personalizado',
  standalone: true,
  imports: [HeaderComponent, FormsModule, FontAwesomeModule, CommonModule],
  templateUrl: './eliminatorias-personalizado.component.html',
  styleUrl: './eliminatorias-personalizado.component.css'
})
export class EliminatoriasPersonalizadoComponent {
  equipos: string[] = []
  equiposFinal: string[] = []
  ...
  constructor(private route: ActivatedRoute, private router: Router) {  
    this.route.params.subscribe(params => {  
      ...
      this.equiposFinal = JSON.parse(params['final'])
        ...
        if(ganadorIdaVuelta[partido.equipo1] === ganadorIdaVuelta[partido.equipo2]){
          ...
          if (this.final) { 
            if (partido.ganador){
              ...
              } else if((this.equipos.length + this.equiposFinal.length) === 7){
                if (this.k === 1){
                  this.k++
                  this.generarPartidos(this.equipo1.concat(partido.ganador))
                } else if (this.k === 2){
                  this.k++
                  this.generarPartidos(this.equiposFinal.concat(partido.ganador))
                } else if (this.k === 3){
                  this.equipoGanador = partido.ganador
                  this.toggleModal()
                }
              } else {
                  const partidoFinal = this.equiposFinal.concat(partido.ganador)
                  this.j++;
                  if(this.j !== 2){
                    this.generarPartidos(partidoFinal)
                  }
                  if (this.j === 2){
                    this.equipoGanador = partido.ganador
                    this.toggleModal()
